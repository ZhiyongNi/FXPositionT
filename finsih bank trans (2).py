...
#先把余额不足的填满
low_limit_bank = ["德意志法兰克福","中行法兰克福","花旗爱尔兰","招行香港","工行路桥境内外币支付账户","工行路桥一般清算账户","中行路桥一般清算账户","交行台州"]
...
#找出有账户盈余超出的银行
up_limit_bank = ["德意志法兰克福","中行法兰克福","花旗爱尔兰","工行路桥境内外币支付账户","中行路桥一般清算账户","交行台州"]
adjust_up_limit = {}
...
adjust_up_limit_df = pd.DataFrame(list(adjust_up_limit.values()),index = adjust_up_limit.keys(),columns = ["超出部分"]).sort_values("超出部分",ascending=False)
adjust_low_limit_df = pd.DataFrame(list(adjust_low_limit.values()),index = adjust_low_limit.keys(),columns = ["不足部分"]).sort_values("不足部分",ascending=False)
#将要转移的单独列出，而后排序分配
...
for give_bank in adjust_up_limit_df.index:
    ...
    if(give_money+rule.loc[recieve_bank,"current"]<= rule.iloc[7,2]):
        ...
        if(rule.loc[recieve_bank,"current"]!=rule.iloc[7,2]):
            ...
            df3=pd.DataFrame([[give_bank,recieve_bank,rule.iloc[7,2]-rule.loc[recieve_bank,"current"]]],columns=['from','to','value'])
            df2=df2.append(df3, ignore_index=True)
